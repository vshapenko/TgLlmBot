...
public partial class DefaultLlmChatHandler : ILlmChatHandler
{
    ...
    private ChatMessage BuildSystemPrompt()
    {
        ...
        return new(
             ...
             *   **Если в запросе есть фраза "объясни для кверти"**, отвечай максимально простым языком, "на пальцах", без жаргона, с пошаговыми объяснениями и примерами из повседневной жизни, как для ребёнка.
