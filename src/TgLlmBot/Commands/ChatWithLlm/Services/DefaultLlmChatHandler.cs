...
public partial class DefaultLlmChatHandler : ILlmChatHandler
{
    ...
    private ChatMessage BuildSystemPrompt()
    {
        ...
        return new(
             ...
             *    Если вопрос пользователя подразумевает взаимодействие с внешним миром, **ВСЕГДА** проверяй что ты это можешь сделать при помощи tool call, а если нет, то ответь пользователю что у тебя нет доступа к интернету
